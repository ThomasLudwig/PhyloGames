...
function buildEspeceTable(array) {
  Object.keys(especes).forEach((k) => delete especes[k]);
...
function getGroupNameForEspece(e) {
  for (const nom in GROUPES) if (GROUPES[nom](e)) return nom;
  return null;
}
...
function buildGroups() {
  ...
  for (const nom in GROUPES) {
    ...
    for (const id in especes) {
      ...
      if (GROUPES[nom](esp)) groups[nom].push(esp);
    }
  }
  return groups;
}
...
function getSelectedGroupes() {
  return [...document.querySelectorAll('input[name="groupes"]:checked')].map(
    (input) => input.value,
  );
}
...
function getGroupIconHtml(groupName) {
  const safe =
    GROUP_ICON_SAFE_NAMES[groupName] ||
    groupName.toLowerCase().replace(/[^a-z0-9]/g, "_");
  return (
    "<img class='icon-order' src='/static/icons/" +
    safe +
    ".svg' " +
    "onerror=\"handleIconOrderError(this, '" +
    safe +
    "')\">"
  );
}
...
function tirageGroupes(groups, choisis, total = 10) {
  let pool = [];
  choisis.forEach((g) => {
    if (groups[g]) pool = pool.concat(groups[g]);
  });
...
function afficherResultats(tirage) {
  ...
  document.getElementById("zoneConfiguration").style.display = "none";

  listeTaxIds = tirage.map((e) => e.id);
  contenuFichier = listeTaxIds.join(",");

  console.log("TaxIds envoyés à Flask :");
  ...
  document.getElementById("btnCorrection").style.display = "inline-block";
  document.getElementById("btnSolution").style.display = "inline-block";
  // hide any test-mode buttons during a draw
  document
    .querySelectorAll(".test-mode")
    .forEach((b) => (b.style.display = "none"));

  const noms = tirage.map((e) => e.toString()).join("<br>");
  ...
  fetch("http://127.0.0.1:5000/prune", {
    ...
    body: JSON.stringify({ tax_ids: listeTaxIds }),
  }).then((response) => {
      console.log("Réponse Flask :", response.status);

      if (!response.ok) throw new Error("Erreur Flask : " + response.status);
    ...
    .then((data) => {
      ...
      solutionURL = data.solution;
      mappingFile = data.mapping;
      equivalentsFile = data.equivalents;
      cladesFile = data.clades;
      return fetch(mappingFile + "?t=" + Date.now());
    })
    .then((r) => {
      console.log("Chargement mapping.json :", r.status);
      return r.json();
    })
    .then((mapping) => {
      mappingCourant = mapping;
      console.log("Mapping reçu :");
      console.table(mapping);
      return fetch(equivalentsFile + "?t=" + Date.now());
    })
    .then((r) => {
      console.log("Chargement equivalents.json :", r.status);
      return r.json();
    })
    .then((eq) => {
      equivalences = eq;
      ...
      console.log(eq);
      return fetch(cladesFile + "?t=" + Date.now());
    }).then((r) => {
      console.log("Chargement clades.json :", r.status);
      return r.json();
    }).then((c) => {
      clades = c;
      console.log("Clades :");
      console.log(c);
      console.log("=================================");
      console.log("SOLUTION DU QUIZ");
      console.log("=================================");
      Object.keys(mappingCourant)
        .sort((a, b) => a - b)
        .forEach((numero) => {
          const taxid = mappingCourant[numero];
          const e = especes[taxid];
          if (e) 
            console.log(numero + " -> " + e.commonName + " (" + e.latinName + ") [" + taxid + "]");
        });
      console.log("=================================");
    ...
    .catch((error) => {
      ...
      document.getElementById("resultats").innerHTML += "<br><br><b>Erreur Flask</b>";
    });
}
// =========================
...
function genererQuiz(mapping) {
  ...
  quiz.innerHTML = "";
  const listeEspeces = Object.values(mapping);
  const modeDifficulte = getModeDifficulte();
  const especesTriees = [...listeEspeces].sort((a, b) => {
    const ea = especes[a];
    const eb = especes[b];
  ...
  for (const numero in mapping) {
    const taxid = especesTriees[index];
    ordreUtilisateur[numero] = taxid;
    const e = especes[taxid];
    const ligne = document.createElement("div");
    ligne.className = "ligne-quiz";
    ligne.dataset.numero = numero;
    ligne.dataset.taxid = taxid;
    ligne.draggable = true;
    const label = e ? `${e.commonName} (${e.latinName})` : `TaxID ${taxid}`;
    ...
    ligne.innerHTML =
      ...
      "<span class='nom'>" + iconHtml + labelText + "</span>";

    ligne.addEventListener("dragstart", () => { dragged = ligne; });
    ligne.addEventListener("dragover", (e) => { e.preventDefault();});

    ligne.addEventListener("drop", () => {
      if (!dragged || dragged === ligne) 
        return;
      const nom1 = dragged.querySelector(".nom").innerHTML;
      const nom2 = ligne.querySelector(".nom").innerHTML;
      dragged.querySelector(".nom").innerHTML = nom2;
      ligne.querySelector(".nom").innerHTML = nom1;
      const tax1 = dragged.dataset.taxid;
      const tax2 = ligne.dataset.taxid;
      dragged.dataset.taxid = tax2;
      ligne.dataset.taxid = tax1;
      ordreUtilisateur[dragged.dataset.numero] = tax2;
      ordreUtilisateur[ligne.dataset.numero] = tax1;
    });
...
function corrigerQuiz() {
  const lignes = document.querySelectorAll(".ligne-quiz");
  let score = 0;
  const dejaValide = new Set();
  const reponsesUtilisateur = {};
  lignes.forEach((ligne) => { reponsesUtilisateur[ligne.dataset.numero] = ligne.dataset.taxid; });

  lignes.forEach((ligne) => {
    ligne.style.backgroundColor = "";
    const numero = ligne.dataset.numero;
    const bonneReponse = mappingCourant[numero];
    const valeur = ligne.dataset.taxid;
    let correct = false;
    if (valeur === bonneReponse)
      correct = true;
    else {
      const soeurs = equivalences[bonneReponse] || [];
      if (soeurs.includes(valeur)) 
        correct = true;
    }

    if (correct) {
      ligne.style.backgroundColor = "#90EE90";
      dejaValide.add(numero);
      score++;
    } else 
      ligne.style.backgroundColor = "#FFB6B6";
  });

  const positionParTaxid = {};

  for (const numero in reponsesUtilisateur) 
    positionParTaxid[reponsesUtilisateur[numero]] = parseInt(numero, 10);

  const toutesLesCladesContigues = clades.every((clade) => {
    const positions = clade
      .map((taxid) => positionParTaxid[taxid])
  ...
  if (toutesLesCladesContigues) {
    ...
    lignes.forEach((ligne) => { ligne.style.backgroundColor = "#90EE90";});
    document.getElementById("score").innerHTML =
      "<h3>Score : " + score + " / " + score + "</h3>" +
      "<b>Ordre valide selon la topologie de l'arbre.</b>";
    document.getElementById("btnNouvellePartie").style.display = "inline-block";
    return;
  }
  ...
  clades.forEach((clade) => {
    ...
    for (const numero in mappingCourant) 
      if (clade.includes(mappingCourant[numero])) 
        numerosDuClade.push(numero);

    let cladeValide = true;
    numerosDuClade.forEach((numero) => {
      const attendu = mappingCourant[numero];
      const obtenu = reponsesUtilisateur[numero];
      if (!obtenu) {
        cladeValide = false;
        return;
      }

      const equivalentes = equivalences[attendu] || [];
      if (obtenu !== attendu && !equivalentes.includes(obtenu)) 
        cladeValide = false;
    });

    if (cladeValide) {
      numerosDuClade.forEach((numero) => {
        if (!dejaValide.has(numero)) {
          dejaValide.add(numero);
          score++;
          const ligne = document.querySelector(".ligne-quiz[data-numero='" + numero + "']");
          if (ligne) 
            ligne.style.backgroundColor = "#90EE90";
        }
      });
  ...
  const total = Object.keys(mappingCourant).length;
  if (score > total) 
    score = total;
...
function voirSolution() {
  ...
  for (const numero in mappingCourant) {
    ...
    if (e) 
      html +=
        "<b>" + numero + "</b> → " + e.commonName + " (" + e.latinName + ")" +
        "<span class='espece-details'>" + e.reign + " / " + e.clas + " / " + e.order + "</span><br>";
    else 
      html += "<b>" + numero + "</b> → TaxID " + taxid + "<br>";
  }

  html +=
    "<br><h3>Arbre corrigé</h3>" +
    "<img src='" + solutionURL + "?t=" + Date.now() + "' style='max-width:100%;border:1px solid #000'>";
...
function getNombreTirage() {
  return parseInt(document.getElementById("selectTirage").value);
}
// =========================
// Nettoyage interface
// =========================
...
function nouvellePartie() {
  nettoyerInterface();
  document.getElementById("zoneConfiguration").style.display = "block";
  ...
  document.getElementById("btnCorrection").style.display = "none";
  document.getElementById("btnSolution").style.display = "none";
  document.getElementById("btnNouvellePartie").style.display = "none";
}
// =========================
// Tirage simple
// =========================
...
function lancerTirage() {
  loadCSV(function (rows) {
    ...
    const choisis = getSelectedGroupes();
    if (choisis.length === 0) {
    ...
    const tirage = tirageGroupes(groups, choisis, nombre);

    if (tirage.length === 0) {
      alert("Aucune espèce trouvée dans les groupes sélectionnés.");
      return;
    }

    afficherResultats(tirage);
  });
}
// Bouton de correction
document.getElementById("btnCorrection").addEventListener("click", corrigerQuiz);
// Bouton solus
document.getElementById("btnSolution").addEventListener("click", voirSolution);
// Bouton pour telecharger le tirage
document.getElementById("btnDownload").addEventListener("click", function () {
  const blob = new Blob([contenuFichier], {type: "text/plain",});
  const lien = document.createElement("a");
  lien.href = URL.createObjectURL(blob);
  lien.download = "taxons.txt";
  lien.click();
...
function lancerTestLineaire() {
  const taxIdsTest = ["1", "2", "3", "4", "5", "6", "7", "8"];
  ...
  console.log("MODE TEST ARBRE LINEAIRE");
  document.getElementById("zoneConfiguration").style.display = "none";
  ...
  fetch("http://127.0.0.1:5000/prune", {
    ...
    console.log("Réponse Flask :", response.status);
    if (!response.ok) 
      throw new Error("Erreur Flask : " + response.status);
    return response.json();
  }).then((data) => {
    console.log("Fichiers générés :");
    console.table(data);
    document.getElementById("tree-container").innerHTML =
      "<img src='" + data.tree + "?t=" + Date.now() + "' style='max-width:100%;height:auto;'>";
    solutionURL = data.solution;
    mappingFile = data.mapping;
    equivalentsFile = data.equivalents;
    cladesFile = data.clades;
...
function lancerTirageAll() {
  loadCSV(function (rows) {
    ...
    choisis.forEach((groupe) => {
      if (groups[groupe]) toutesEspeces = toutesEspeces.concat(groups[groupe]);      
    });

    // Get the number of species to draw
    const nombre = getNombreTirage();
    // Draw random species
    const tirage = tirageAleatoire(toutesEspeces, nombre);
    console.log("MODE TIRAGE TOUTES ESPECES", tirage.length);
    afficherResultats(tirage);
  });
}
