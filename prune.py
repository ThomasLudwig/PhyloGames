...
if len(keep) == 0:
  raise ValueError("Aucun taxon valide trouvé dans l'arbre")
...
for node in tree.traverse():
  ...
  if len(enfants) != 2:
    continue
  # feuilles soeurs
  if (enfants[0].is_leaf() and enfants[1].is_leaf()):
    a = enfants[0].name
    b = enfants[1].name
...
# Sauvegarde mapping
with open(os.path.join(args.session, "mapping.json"), "w", encoding="utf-8") as f:
  json.dump( mapping, f, ensure_ascii=False, indent=4)

# Sauvegarde équivalences
with open(os.path.join(args.session, "equivalents.json"),"w",encoding="utf-8") as f:
  json.dump(equivalents, f, ensure_ascii=False, indent=4)

# Sauvegarde clades
with open(os.path.join(args.session,"clades.json"),"w",encoding="utf-8") as f:
  json.dump(clades,f,ensure_ascii=False,indent=4)

# Chargement noms espèces
try:
  with open("static/especes.json","r",encoding="utf-8") as f:
    noms = json.load(f)
except Exception:
  noms = {}
...
nb_especes = len(keep)
largeur = 1200
hauteur = max(500, nb_especes * 80)
# Arbre quiz
tree.render(
    os.path.join(args.session,"tree.png"),
    w=largeur,
    h=hauteur,
    units="px",
    tree_style=ts
...
solution_tree.render(
  os.path.join(args.session, "solution.png"),
  w=largeur,
  h=hauteur,
  units="px",
  tree_style=ts
